...
// weighted by the precomputed Fresnel factor. (Same technique as lens-orb.)
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Force opaque alpha — the canvas is transparent (page bg shows around
    // the orb) but the orb itself must NOT mix with the page bg through the
    // body. uRefractionPower / fresnel mix above only drives the RGB blend
    // between refract and reflect; the final alpha is always 1 so the orb
    // reads as a solid object, same as the Hero LensScene.
    gl_FragColor.a = 1.0;
  }
`;
